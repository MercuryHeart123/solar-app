...
const Overall: React.FC<OverallProps> = ({ data, billBaht, monthKWh }) => {
  ...
  const formattedBill = useMemo(() => {
    const value = typeof billBaht === "number" ? billBaht : 3200;
        ...
        tooltip: {
          callbacks: {
            label: function (context: any) {
              ...
              const pct =
                totalKWh > 0 ? ((value / totalKWh) * 100).toFixed(1) : "0.0";
    ...
    <div className="grid gap-6 md:grid-cols-[minmax(260px,340px)_1fr] items-center">
      <div className="relative">
        <div className="h-[260px] w-full">
          <Doughnut data={dataSet} options={options} plugins={[centerPlugin]} />
        </div>
      </div>
      ...
      <div className="grid gap-3 content-start">
        ...
        <ul className="grid gap-2">
          {chartData.map((d, i) => {
            const ratio = totalKWh > 0 ? (d.kWh / totalKWh) * 100 : 0;
            return (
              <li
                key={`${d.name}-${i}`}
